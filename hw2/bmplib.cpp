...
int readRGBBMP(const char *filename, unsigned char inputImage[][SIZE][RGB]) {
    ...
    // cout << (int)tempImage[0][0][0] << "," << (int)tempImage[0][0][1] << ","
    // << (int)tempImage[0][0][2] << endl;
    fclose(file);
...
int readGSBMP(const char *filename, unsigned char inputImage[][SIZE]) {
    ...
    //  cout << (int)tempGSImage[0][0][0] << "," << (int)tempGSImage[0][0][1] <<
    //  "," << (int)tempGSImage[0][0][2] << endl;
    fclose(file);
...
//   for(i=0; i < SIZE; i++){
//     for(j=0; j < SIZE; j++){
//       ifile.read( reinterpret_cast<char *>(&image[SIZE-1-i][j]),
//       sizeof(uint8));
//     }
//   }
//   return 0;
...
//       ofile.write( reinterpret_cast<char *>(&outputImage[SIZE-1-i][j]),
//       sizeof(uint8) );
//     }
//   }
