...
class Parser {
  public:
     ...
     *        filling in the referenced vectors. The vectors will be cleared
     * before adding the new objects.
     *
     * @param is Input stream (ifstream, stringstream, etc.) to read in the data
     * from
     * @param content Vector of Content pointers to be filled
     * @param users Vector of User pointers to be filled
     *
     * @throw ParserError if there is an error in extracting the expected
     * data/format
     */
    virtual void parse(std::istream &is, std::vector<Content *> &content,
