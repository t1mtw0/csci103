...
class Player {
  public:
    Player(int p) {
        int np = p;
        playerScores = new int[p];
        playerNames = new string[p];
    }
    ~Player() {
        delete[] playerScores;
        delete[] playerNames;
    ...
    string getName(int n) { return playerNames[n]; }
    int getScore(int n) { return playerScores[n]; }
    void updateScore(int n, int s) { playerScores[n] = s; }
    void printAllScores() {
...
int main() {
    ...
    while (!done) {
        ...
        int r1, c1, r2, c2;
        cout << players.getName(currPlayer)
        ...
        if (status) {
            ...
            players.updateScore(currPlayer, players.getScore(currPlayer) + 1);
            if (players.getScore(currPlayer) > players.getScore(maxPlayer)) {
        ...
        cout << endl;
        players.printAllScores();
        done = board.isGameOver();
    }
    board.printGameBoard();
    cout << "Game over!" << endl;
    cout << "Winner: " << players.getName(maxPlayer) << endl;
