...
void printGameBoard(string board[][NCOL], bool guessed[][NCOL], int nr, int nc);
void initGameBoard(string board[][NCOL], bool guessed[][NCOL], int nr, int nc);
bool checkMatch(string board[][NCOL], bool guessed[][NCOL], int nr, int nc,
                int r1, int c1, int r2, int c2);
bool isGameOver(bool guessed[][NCOL], int nr, int nc);
...
void printGameBoard(string board[][NCOL], bool guessed[][NCOL], int nr,
                    int nc) {
    for (int r = 0; r < nr; r++) {
        for (int c = 0; c < nc; c++) {
            if (guessed[r][c]) {
...
int main() {
    srand(time(0));
    // Board is MAX of NROW by NCOL (not we may use less)
    ...
    initGameBoard(board, guessed, nr, nc);
    bool done = false;
    int currPlayer = 0;
    int maxPlayer = 0;
    while (!done) {
        // take 1 turn in each iteration
        printGameBoard(board, guessed, nr, nc);
        int r1, c1, r2, c2;
        ...
        cin >> c1 >> r2 >> c2;
        bool status = checkMatch(board, guessed, nr, nc, r1, c1, r2, c2);
        if (status) {
